...
impl DataSourceType {
    ...
    fn possible_paths(&self) -> Vec<String> {
        match self {
            DataSourceType::Messages => vec!["~/Library/Messages/chat.db".to_string()],
            ...
            DataSourceType::KnowledgeC => {
                vec!["/private/var/db/CoreDuet/Knowledge/knowledgeC.db".to_string()]
            }
...
pub fn extract_all_data(config: ExtractionConfig) -> Result<ExtractionReport, ExtractionError> {
    ...
    for source_type in config.enabled_sources {
        ...
        let result = match collectors::create_collector(collector_type, &core_config, &unified_db)
        {
            Ok(mut collector) => match collector.run() {
                Ok(extraction_result) => {
                    total_added += extraction_result.records_added as u64;
                    total_skipped += extraction_result.records_skipped as u64;

                    SourceResult {
                        source_type,
                        source_name: source_type.name(),
                        records_added: extraction_result.records_added as u64,
                        records_skipped: extraction_result.records_skipped as u64,
                        success: true,
                        error_message: None,
                    }
                }
...
pub fn extract_single_source(
    ...
    let result = match collectors::create_collector(collector_type, &core_config, &unified_db) {
        Ok(mut collector) => match collector.run() {
            Ok(extraction_result) => SourceResult {
                source_type,
                source_name: source_type.name(),
                records_added: extraction_result.records_added as u64,
                records_skipped: extraction_result.records_skipped as u64,
    ...
    let records_added = result.records_added;
    let records_skipped = result.records_skipped;
...
pub fn get_database_stats(output_dir: String) -> Result<DatabaseStats, ExtractionError> {
    ...
    for (table, col) in [
        ...
        ("podcast_episodes", "last_played_date"),
    ] {
