#!/usr/bin/env python3
"""
Stats viewer for digital self tracking data.

Usage:
    python stats.py              # Show all stats
...
DB_PATH = Path(__file__).parent / "unified.db"


def get_db():
    if not DB_PATH.exists():
        print("No database found. Run ./run.sh first.")
        exit(1)
...
def show_week(conn):
    ...
    for day, total, apps in days:
        bar = '█' * int((total or 0) / 3600)  # 1 block per hour
        print(f"    {day}  {format_duration(total):>10}  {bar}")

    # Weekly totals by app
...
def show_podcasts(conn):
    ...
    # Top shows by episodes played
    print_subheader("Top Shows by Episodes Played")
    shows = conn.execute("""
        SELECT
            show_title,
            COUNT(*) as episodes,
...
def show_messages(conn):
    ...
    if count == 0:
        ...
        print("  Grant Full Disk Access to Terminal and run ./run.sh again.")
        return
...
def main():
    parser = argparse.ArgumentParser(description='View digital self tracking stats')
    parser.add_argument('view', nargs='?', default='all',
                       choices=['all', 'today', 'week', 'apps', 'browsing', 'podcasts', 'messages', 'bluetooth'],
                       help='Which stats to show')
    args = parser.parse_args()

    conn = get_db()

    print("\n" + "╔" + "═" * 58 + "╗")
    print("║" + "DIGITAL SELF STATS".center(58) + "║")
    ...
    if args.view == 'all':
        ...
        show_apps(conn)
    elif args.view == 'browsing':
        show_browsing(conn)
    elif args.view == 'podcasts':
        show_podcasts(conn)
    elif args.view == 'messages':
        show_messages(conn)
    elif args.view == 'bluetooth':
        show_bluetooth(conn)
